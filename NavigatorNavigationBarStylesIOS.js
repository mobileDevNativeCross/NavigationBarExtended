...
var Stages = {
  Left: {
    ...
    SearchButton: merge(BASE_STYLES.SearchButton, { opacity: 0 }),
    RightButton: merge(BASE_STYLES.RightButton, { opacity: 0 }),
    Style: merge(BASE_STYLES.Style, { opacity: 0 }),
  },
  Center: {
    Title: merge(BASE_STYLES.Title, { left: 0, opacity: 1 }),
    LeftButton: merge(BASE_STYLES.LeftButton, { left: 0, opacity: 1 }),
    SearchButton: merge(BASE_STYLES.SearchButton, { opacity: 1 }),
    RightButton: merge(BASE_STYLES.RightButton, { opacity: 1 }),
    Style: merge(BASE_STYLES.Style, { opacity: 1 }),
  },
  Right: {
    Title: merge(BASE_STYLES.Title, { left: SCREEN_WIDTH / 2, opacity: 0 }),
    LeftButton: merge(BASE_STYLES.LeftButton, { left: 0, opacity: 0 }),
...
function buildSceneInterpolators(startStyles, endStyles) {
  return {
    ...
    SearchButton: buildStyleInterpolator({
      opacity: {
        type: 'linear',
        from: startStyles.SearchButton.opacity,
        to: endStyles.SearchButton.opacity,
        min: 0,
        max: 1,
        round: opacityRatio,
      },
      left: {
        type: 'linear',
        from: startStyles.SearchButton.left,
        to: endStyles.SearchButton.left,
        min: 0,
        max: 1,
        extrapolate: true,
      },
